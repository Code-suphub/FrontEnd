...
@Service
public class SquareCommentServiceImpl extends ServiceImpl<SquareCommentMapper, SquareComment>
  ...
  @Override
  public List<SquareComment> getCommentByIdOrder(Integer postId, Integer parentId, String orderBy) {
    // 条件查询 查询文章为postId的根评论
    QueryWrapper<SquareComment> wrapper = new QueryWrapper<>();
    wrapper.select().orderByDesc(orderBy);
    // TODO 修改为常量
    if(postId!=-1) { // -1 表示当前找的是回复的回复
